...
Verified 2026-09-06. Scope: four repositories. Branch in each repository: `codex/retrieval-preferences-complete`. All four implementation branches are now merged into `origin/main` and the local main checkouts. AI Chat was restarted through its existing launchd service on port 4174 and its new preference UI is verified. The user subsequently selected local dogfooding with docs.kujolang.ai; the persistent loopback service and AI Chat Documentation preset are now configured and verified. Unrelated shared-checkout work and the separate AI Chat soak were preserved.
...
### Local dogfood activation · 2026-09-06

The user selected a local service and the published docs.kujolang.ai corpus. RAG `85743f8` adds the sitemap snapshot/extraction and service runner; AI Chat `8cfc645` preserves public citation URLs and supports a bounded local timeout. 100 pages were indexed into 184 chunks, with zero ingestion errors. `com.kujo.rag.docs` starts at login on 127.0.0.1:8787, requires a private bearer token and uses namespace `kujo_docs`. AI Chat's Documentation preset is enabled and persisted across restart. Real queries returned installation/quickstart and owned-agent-project pages with source URLs. Unauthenticated queries returned 401.

AI Chat's full updated suite passed 384 tests with one platform skip; two extraction tests and four focused adapter tests passed. Cold queries measured about 20–45 seconds on the shared host, so the local adapter timeout is 60 seconds. No claim of language-selection savings is made for this corpus: its pages do not declare equivalent-example groups and retain their Kujo examples. The old pending endpoint/corpus decision is resolved. The pre-existing overdue threat-model review remains a public release-readiness issue, not a claim of production certification for this private installation. Operator instructions and refresh commands are in RAG `docs/LOCAL_KUJO_DOCS.md`.
